...
# fill this out, this is a dictionary of th
...
species_pairs = list(combinations(good_species_list.keys(), 2))
...
for species_pair in species_pairs:
    ...
    # calculate correlation between species across samples
...
ax.plot(x_range, y_fit_range, c='k', lw=2.5, linestyle='--', zorder=2)

#ax.title("Distance-de", fontsize=14)
ax.axhline(y=0, color='k', linestyle=':', lw=2, alpha = 0.8, zorder=1)

ax.set_xlabel('Phylogenetic distance' , fontsize = 12)
ax.set_ylabel('Correlation between the proportion of\nnonsynonymous substitutions across pathways' , fontsize = 12)
